...
const WS4KP_HAZARDS = process.env.WS4KP_HAZARDS || true; // show hazards
const WS4KP_LATEST = process.env.WS4KP_LATEST || true; // how latest forcast
const WS4KP_HOURLY = process.env.WS4KP_HOURLY || false; //show hourly forcast
const WS4KP_HOURLYGRAPH = process.env.WS4KP_HOURLYGRAPH || true; // show hourly graphs
const WS4KP_TRAVEL = process.env.WS4KP_TRAVEL || false; // show travel forcast
const WS4KP_REGIONAL = process.env.WS4KP_REGIONAL || true; // show regional forcast
const WS4KP_LOCAL = process.env.WS4KP_LOCAL || true; // how local forcast
const WS4KP_EXTENDED = process.env.WS4KP_EXTENDED || true; // show extended forcast
const WS4KP_ALMANAC = process.env.WS4KP_ALMANAC || true; // boolean to show almanac
const WS4KP_SPC = process.env.WS4KP_SPC || true; // &spc-outlook-checkbox=true
const WS4KP_RADAR = process.env.WS4KP_RADAR || true; // show radar
const WS4KP_WIDE = process.env.WS4KP_WIDE || false; // show in wide screen
const WS4KP_KIOSK = process.env.WS4KP_KIOSK || false; // display in kiosk
const WS4KP_STICKYHKIOSK = process.env.WS4KP_STICKYHKIOSK || false; // set as sticky kiosk
const WS4KP_CUSTOMFEEDEN = process.env.WS4KP_CUSTOMFEEDEN || false; // show custom feed
const WS4KP_SPEED = process.env.WS4KP_SPEED || "1.0"; // speed
const WS4KP_SCANLINEMODE = process.env.WS4KP_SCANLINEMODE || "auto"; // show scanline mode
const WS4KP_UNITS = process.env.WS4KP_UNITS || "auto"; // units
const WS4KP_TXTLOC = process.env.WS4KP_TXTLOC || ""; // location in text form
const WS4KP_CUSTOMFEEDSTR = process.env.WS4KP_CUSTOMFEEDSTR || ""; // custom feed text
const WS4KP_SHARELINK = process.env.WS4KP_SHARELINK || ""; // share link
const WS4KP_SCANLINEEN = process.env.WS4KP_SCANLINEEN || false; // enable scanlines
const WS4KP_MEDIAVOLUME = process.env.WS4KP_MEDIAVOLUME || "0.75"; // media volume
const WS4KP_LOCQUERY = process.env.WS4KP_LOCQUERY || "" // location query
